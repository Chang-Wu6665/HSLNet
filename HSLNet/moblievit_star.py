...
from HSLNet.transformer import TransformerEncoder
from HSLNet.model_config_star2 import get_config
import torch
import torch.nn as nn
...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        # 新增layer6
        self.layer_6, out_channels = self._make_layer(input_channel=out_channels, cfg=model_cfg["layer6"]
        )
    ...
    def _make_layer(self, input_channel, cfg: Dict) -> Tuple[nn.Sequential, int]:
        ...
        if block_type.lower() == "star":  # 新增star类型判断
            return self._make_star_layer(input_channel=input_channel, cfg=cfg)
        elif block_type.lower() == "mobilevit":
    ...
    @staticmethod
    # 新增Star层构建方法
    def _make_star_layer( input_channel: int, cfg: Dict) -> [nn.Sequential, int]:
        blocks = []
        ...
        # 添加Star Blocks
        depth = cfg.get("depth", 3)  # 默认4个StarBlock
        ...
        return nn.Sequential(*blocks), input_channel
    ...
    def forward_features(self, x: Tensor) -> Tensor:
        ...
        x4 = self.layer_4(x3)
        x5 = self.layer_5(x4)
        x6 = self.layer_6(x5)  # 新增的StarBlock层
        layer_features = [x1, x2, x3, x4, x5, x6]
